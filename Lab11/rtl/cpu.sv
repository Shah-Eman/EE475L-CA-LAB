// Lab 11 - 3-stage pipelined RISC-V processor (forwarding + load-use stall
// + branch flush, identical datapath to Lab 10) with:
//   * memory pre-load from hex files (IMEM_FILE / DMEM_FILE), and
//   * four debug outputs exposing the sorted array words so the FPGA top
//     level can drive the seven-segment displays.
module cpu #(
    parameter IMEM_FILE = "",
    parameter DMEM_FILE = "",
    ...
    input  logic        clk,
    input  logic        rst,
    output logic [31:0] arr_word0,
    output logic [31:0] arr_word1,
    output logic [31:0] arr_word2,
    output logic [31:0] arr_word3
);
    ...
    IF_STAGE #(.IMEM_FILE(IMEM_FILE)) IF_STAGE (
        .clk         (clk),
        .rst         (rst),
        .pc_en       (pc_en),
        .pc_next     (pc_next),
