...
module cpu_tb();
    ...
    initial begin
        ...
        // --- Forwarding: back-to-back ALU dependency ---
        reset_memories();
        `IMEM_PATH.memory[0] = {12'd5, 5'd0, `FNC_ADD_SUB, 5'd1, `OPC_ARI_ITYPE};
        `IMEM_PATH.memory[1] = {7'd0, 5'd1, 5'd1, `FNC_ADD_SUB, 5'd2, `OPC_ARI_RTYPE};
        reset_cpu();
        wait_rf(2, 32'd10, "Forward ALU RAW");

        // --- Load-use stall ---
        reset_memories();
        `REGFILE_PATH.Registers[3] = 32'h0000_0100;
        `DMEM_PATH.memory[32'h100] = 32'hCAFE_BABE;
        `IMEM_PATH.memory[0] = {12'd0, 5'd3, 3'b010, 5'd4, `OPC_LOAD};
        `IMEM_PATH.memory[1] = {7'd0, 5'd0, 5'd4, `FNC_ADD_SUB, 5'd5, `OPC_ARI_RTYPE}; // add x5, x4, x0
        reset_cpu();
        wait_rf(5, 32'hCAFE_BABE, "Load-use stall");
        ...
        all_tests_passed = 1;
        $display("All Lab 10 pipeline tests passed!");
        $finish();
    end
endmodule
